...
export default function TroisPions({ onBack, stats, onStats, mode }) {
  ...
  const [selPion, setSelPion] = useState(null);
  ...
  function finishTurn(next, nh, result, wl, nextTurn) {
    setBoard(next); setHist(nh);
    if (result === 'win')  { setWinner('j1'); setWinLine(wl); onStats('wins'); return; }
    if (result === 'lose') { setWinner('j2'); setWinLine(wl); onStats('losses'); return; }
    if (result === 'draw') { setWinner('draw'); onStats('draws'); return; }
    setTurn(nextTurn);
    if (mode === 'ai' && nextTurn === 'j2') doAI(next, nh);
  }
  ...
  function onCell(i) {
    ...
    const phase = getPhase(board, currentPlayer);

    if (phase === 'place') {
      if (board[i] !== null) return;
      const next = [...board]; next[i] = currentPlayer;
      const { nh, result, winLine: wl } = applyAndCheck(next, currentPlayer, hist);
      const nextTurn = currentPlayer === 'j1' ? 'j2' : 'j1';
      finishTurn(next, nh, result, wl, nextTurn);
    } else {
      if (selPion === null) {
        if (board[i] === currentPlayer) setSelPion(i);
      } else {
        if (i === selPion) { setSelPion(null); }
        else if (board[i] === currentPlayer) { setSelPion(i); }
        else if (board[i] === null) {
          const next = [...board]; next[selPion] = null; next[i] = currentPlayer;
          const { nh, result, winLine: wl } = applyAndCheck(next, currentPlayer, hist);
          setSelPion(null);
          const nextTurn = currentPlayer === 'j1' ? 'j2' : 'j1';
          finishTurn(next, nh, result, wl, nextTurn);
  ...
  const phase = getPhase(board, turn);
  const wlc = winLine ? [POSITIONS[winLine[0]], POSITIONS[winLine[2]]] : null;
  const phaseMsg = !winner
    ? phase === 'place'
      ? `${turn === 'j1' ? 'Joueur 1' : mode === 'ai' ? 'IA' : 'Joueur 2'} — posez vos pions (${countPions(board, turn)}/3)`
      : `${turn === 'j1' ? 'Joueur 1' : mode === 'ai' ? 'IA' : 'Joueur 2'} — déplacez un pion`
    : '';
